#!/bin/bash
# videomae on MINDS dataset with pre-train on kinetics 10x (1 per seed) with specific augmentations minus random rotation
...
do
    ...
    experiment_name="new_videomae_kinetics_all_best_augs_minusRR_seed_${seed}"
    ...
    CUDA_VISIBLE_DEVICES=0 python train.py \
        ...
        --random_sample \
        --dataset "minds" \
        -t "normalize" "color_jitter" "random_perspective" \
        -tp "color_jitter_${cj}_${cj}_${cj}_${cj}" "random_perspective_${rp}"\
        --mixup \
        --mixup_alpha "$mu" \
