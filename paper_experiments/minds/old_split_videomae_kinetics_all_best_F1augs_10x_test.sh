...
NUM_WORKERS=32 # Reduced from 16 to 8 (adjust based on your CPU cores)

# Read each line from the seeds file
while IFS= read -r seed
do
    # Experiment name includes all the augmentations
    experiment_name="videomae_kinetics_all_best_F1augs_seed_${seed}_OLD_SPLIT_TESTING"
    log_file="./logs/${experiment_name}.log"

    # Run the experiment
    echo "Running experiment ${experiment_name}..." | tee -a "$log_file"
    CUDA_VISIBLE_DEVICES=2 python train_test_2.py \
